...
`server/_core/app.ts` exports `createApp(): Express` — the whole API surface (body parsers, `/api/tank01/:endpoint`, `/api/scheduled/*`, `/api/trpc`), with no static serving and no `.listen()`. Three things consume it:
- `server/_core/vercelHandler.ts` — a one-line `export default createApp();`, esbuild-bundled into `api/index.js` (the actual Vercel serverless entry point) by the `bundle:api` script in `package.json`. `api/index.js` (and the scheduled-function bundles below) are gitignored -- they're build output, not source, and `vercel.json`'s `functions` block references them by that generated path. `vercel.json` rewrites all `/api/*` traffic to this one function (preserving the original path, so Express's own router still dispatches correctly) plus an SPA fallback (`/(.*)` → `/index.html`) for wouter's client-side routes.
- `server/_core/vercelScheduled*.ts` (one per scheduled function — 8 of them, against 10 entries in `vercel.json`'s `crons` array, because `historical-season-stats-backfill` is scheduled three times with a different `?year=`) — each a thin wrapper around a single sync function, similarly bundled by `bundle:api` into `api/scheduled/*.js`.
...
Because the API functions are generated, not committed, **the `bundle:api` script must run before any Vercel build** — `.github/workflows/deploy.yml` does this explicitly (`pnpm run bundle:api` then `vercel build --prod` then `vercel deploy --prebuilt --prod`) on every push to `main`. `vercel.json`'s own `buildCommand` (`vite build`) only covers the static client and is not sufficient by itself to produce the API functions -- if Vercel's own Git integration is *also* connected to this repo (separate from the GitHub Actions workflow above), its deploys would use that bare `buildCommand` and ship without a working `api/index.js`, silently overwriting whichever deploy (Action or native) finished last. If the live site seems to be running stale server logic despite a green Actions run, check whether Vercel's native Git integration is enabled for this project and disable it in favor of the Actions workflow.
