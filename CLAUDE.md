...
- `server/_core/vercelScheduled*.ts` (one per cron job in `vercel.json`'s `crons` array) — each a thin wrapper around a single sync function, similarly bundled by `bundle:api` into `api/scheduled/*.js`.
