...
function PlayerPool() {
  ...
  // Same class of bug as the voice recorder's old 150 cap: with no search filter typed
  // in, "All positions" was silently truncating to the first 75 eligible players
  // (alphabetically) out of what's likely several hundred, hiding most of the real
  // pool from anyone just browsing rather than searching for a specific name.
  const input = useMemo(() => ({ search: search.trim() || undefined, position: position || undefined, limit: 300 }), [position, search]);
