...
import { Gavel, Mic, PauseCircle, Search, ShieldCheck, Users } from "lucide-react";
...
function PlayerPool() {
  ...
  // Raised from 300 to 1000 (the endpoint's actual max): now that the eligible pool is
  // correctly scoped to ~606 real active players (after tonight's retired-player
  // filtering fix), even 300 was cutting off roughly half the real list.
  const input = useMemo(() => ({ search: search.trim() || undefined, position: position || undefined, limit: 1000 }), [position, search]);
