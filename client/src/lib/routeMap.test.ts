...
describe("CVC route contract", () => {
  it("includes every required league view and protected workspace", () => {
    ...
    ["/standings", "/live", "/lineup", "/protections", "/draft-recap", "/transactions", "/trades", "/free-agents", "/results", "/history", "/playoffs", "/rules", "/nfl-sites", "/rosters", "/money", "/settings", "/player/:playerId", "/login"].forEach(path => expect(allRoutes).toContain(path));
  });
