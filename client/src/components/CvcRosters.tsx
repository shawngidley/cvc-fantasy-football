...
// Matches CvcOwnerLineup's isDst exactly -- DST rows don't get an injury badge there,
// mirrored here for the same reason (a defense doesn't carry an individual injury
// status the way a rostered player does).
...
function RosterCard({ franchise, mine }: { franchise: any; mine?: boolean }) {
  ...
  const rosterGridCols = "grid-cols-[2.25rem_minmax(0,1fr)_2.25rem_3.75rem_4.5rem]";
  return <section className={mine ? "overflow-hidden rounded-xl bg-white shadow-xl ring-2 ring-cvc-accent" : "overflow-hidden rounded-xl bg-white shadow-xl"}>
    ...
    <div className="border-t border-slate-200">
      <div className={`grid ${rosterGridCols} gap-2 bg-slate-100 px-4 py-2 text-[9px] font-black uppercase tracking-[.08em] text-slate-500`}><span>Pos</span><span>Player</span><span>NFL</span><span className="text-right">Salary</span><span className="text-right">Contract</span></div>
      {roster.isLoading ? <p className="px-4 py-8 text-center text-sm text-slate-400">Loading roster…</p> : players.length ? players.map((item, index) => <div className={`grid ${rosterGridCols} items-center gap-2 px-4 py-2 ${index % 2 ? "bg-slate-50" : "bg-white"}`} key={item.id}><span className={`rounded px-1.5 py-0.5 text-center text-[10px] font-bold ${posTone[item.player?.position] ?? "bg-slate-100 text-slate-700"}`}>{item.player?.position ?? "—"}</span><span className="flex min-w-0 items-center gap-1"><Link href={`/player/${item.player_id}`} className="min-w-0 truncate text-sm font-semibold text-cvc-deep hover:text-cvc-accent">{shortName(item.player?.display_name)}</Link>{(() => { const s = injuryStatuses.statuses.get(normalizePlayerName(item.player?.display_name ?? "")); return !isDst(item.player?.position) && s ? <CvcInjuryBadge status={s} /> : null; })()}</span><span className="text-xs font-semibold text-slate-500">{item.player?.nfl_team ?? "FA"}</span><span className="text-right text-[10px] font-bold text-cvc-deep">{item.contract ? `$${Number(item.contract.salary).toFixed(0)}` : "—"}</span><span className="text-right text-[10px] font-bold uppercase text-slate-600">{contractLabel(item)}</span></div>) : <p className="px-4 py-8 text-center text-sm text-slate-400">No active roster assignments.</p>}
