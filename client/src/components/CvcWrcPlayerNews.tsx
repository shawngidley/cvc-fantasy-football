...
export function CvcWrcPlayerNews() {
  ...
  return <section className="min-h-screen bg-[#06121b] px-3 pb-14 pt-5 text-white sm:px-6"><div className="mx-auto max-w-4xl">
    ...
    <div className="mb-4 rounded-lg border border-yellow-500 bg-yellow-950/60 p-3 font-mono text-[11px] text-yellow-200">
      DEBUG: source={source} | fantasyPros.isLoading={String(fantasyPros.isLoading)} | fantasyPros.isFetching={String(fantasyPros.isFetching)} | fantasyPros.isError={String(fantasyPros.isError)} | fantasyPros.error={String(fantasyPros.error?.message ?? "none")} | data.items.length={String(fantasyPros.data?.items?.length ?? "no data")} | data.source={String(fantasyPros.data?.source ?? "n/a")} | fantasyProsMapped.length={fantasyProsMapped.length} | bySource[source].length={bySource[source].length} | shown.length={shown.length} | isLoading={String(isLoading)} | isUnavailable={String(isUnavailable)}
    </div>
