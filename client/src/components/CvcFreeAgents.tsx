  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    {tab === "manage-bids" ? (
      <div className="grid gap-5">
        <section className="rounded-xl border border-white/10 bg-cvc-deep/60 p-5"><div className="flex items-center gap-2 text-cvc-accent"><DollarSign size={16} /><p className="font-display text-lg uppercase">My claim status</p></div><div className="mt-4 space-y-2">{owner?.franchise ? myBids.data?.length ? myBids.data.map((bid: any) => { const period = Array.isArray(bid.period) ? bid.period[0] : bid.period; const needsConfirmation = bid.status === "pending" && period?.period_type === "free" && period?.status === "open" && !bid.confirmed_at; const isPending = bid.status === "pending"; return <div key={bid.id} className="flex items-center justify-between gap-3 rounded bg-white/5 px-3 py-2 text-sm text-white"><span><b>{bid.player?.[0]?.display_name ?? bid.player?.display_name}</b> · ${bid.amount} · <span className="uppercase text-cvc-accent">{needsConfirmation ? "unconfirmed" : bid.status}</span></span><span className="flex shrink-0 gap-2">{needsConfirmation ? <button onClick={() => confirmClaim.mutate({ bidId: bid.id })} disabled={confirmClaim.isPending} className="rounded bg-amber-500 px-2.5 py-1 text-xs font-bold text-white hover:bg-amber-600">Confirm claim</button> : null}{isPending ? <button onClick={() => cancelClaim.mutate({ bidId: bid.id })} disabled={cancelClaim.isPending} className="rounded bg-white/10 px-2.5 py-1 text-xs font-bold text-white hover:bg-white/20">Cancel</button> : null}</span></div>; }) : <p className="text-sm text-cvc-muted">No CVC waiver claims submitted.</p> : <p className="text-sm text-cvc-muted">Sign in with an owner account to submit and review claims.</p>}</div></section>
      </div>
    ) : (
      <section className="overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="h-1.5 bg-cvc-accent" />
        <div className="flex flex-wrap items-center justify-between gap-3 bg-cvc-deep px-5 py-4 text-white"><div><p className="font-display text-2xl uppercase tracking-[.06em]">{tab === "watchlist" ? "Your watchlist" : tab === "all-players" ? "All players" : "Available players"}</p>{tab === "all-players" || tab === "watchlist" ? <p className="mt-1 text-xs text-white/65">{tab === "all-players" ? "Every CVC-tracked player, rostered or not." : "Players you're tracking for a future claim or trade."}</p> : null}</div><span className="text-xs font-bold uppercase tracking-[.1em] text-cvc-accent">{players.length} shown</span></div>
