...
import { formatKickerEvent, getKickerEventsForPlayer } from "@/lib/espnKickerEvents";
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-[1.5rem] bg-white shadow-2xl ring-1 ring-black/10">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 border-b border-slate-200 px-3 py-5 sm:gap-3 sm:px-7">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3"><TeamLogo name={selected.away} logoUrl={selected.awayLogoUrl} size="lg" className="shrink-0 rounded-xl border-cvc-deep/20"/><div className="min-w-0"><p className="min-w-0 font-display text-sm uppercase leading-tight text-cvc-deep sm:text-3xl">{selected.away}</p><p className="text-[10px] text-slate-500 sm:text-xs">PROJ {awayStats.projTotal.toFixed(1)}</p><div className="mt-1 flex items-center gap-2 text-[9px] text-slate-500 sm:text-[10px]"><span title="Played / Playing now / Yet to play">👥 {awayStats.played} {awayStats.playing} {awayStats.yetToPlay}</span><span title="Total minutes remaining across all your starters' games">⏱ {awayStats.minutesRemaining}</span></div></div></div>
...
function LineupRow({ away, home, slot, points, live, profiles, projections, gameStatus }: { away: any; home: any; slot: string; points: (entry: any) => number | null; live: ReturnType<typeof useCvcTank01LiveScores>; profiles: Record<string, any>; projections: ReturnType<typeof useCvcNFLProjections>["projections"]; gameStatus: NflGameStatusMap }) {
