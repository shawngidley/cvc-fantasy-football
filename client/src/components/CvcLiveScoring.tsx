...
export function CvcLiveScoring() {
  ...
  return <div className="space-y-5">
    ...
    {/* Mobile-only: the other matchups, stacked vertically at full width below the
       selected (default: the owner's own) game's box score, replacing the horizontal
       score-pill strip used on wider screens. Tapping one swaps it into the box score
       above, same as the desktop strip. */}
    {matchups.length > 1 ? <div className="space-y-2 sm:hidden">{matchups.filter(matchup => matchup.id !== selected.id).map(matchup => { const away = hasLiveScores ? total(matchup.awayLineup.filter((entry: any) => isStarterSlot(entry.slot))) : Number(matchup.awayScore); const home = hasLiveScores ? total(matchup.homeLineup.filter((entry: any) => isStarterSlot(entry.slot))) : Number(matchup.homeScore); return <button key={matchup.id} type="button" onClick={() => setSelectedId(matchup.id)} className="flex w-full items-center justify-between gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-2.5 text-left transition hover:border-cvc-accent/60"><span className="flex min-w-0 flex-1 items-center gap-2"><TeamLogo name={matchup.away} logoUrl={matchup.awayLogoUrl} size="sm"/><span className="truncate text-xs font-semibold text-white">{matchup.away}</span></span><span className="flex shrink-0 items-center gap-1.5 px-1"><strong className="font-display text-base text-white">{away.toFixed(1)}</strong><span className="text-[10px] text-cvc-muted">-</span><strong className="font-display text-base text-white">{home.toFixed(1)}</strong></span><span className="flex min-w-0 flex-1 items-center justify-end gap-2"><span className="truncate text-xs font-semibold text-white">{matchup.home}</span><TeamLogo name={matchup.home} logoUrl={matchup.homeLogoUrl} size="sm"/></span></button>; })}</div> : null}
