...
function SectionTable({ group, profiles, live, rules, slots, updateSlot, canEdit }: { group: CvcLineupGroup; profiles: Record<string, Tank01Profile | null>; live: ReturnType<typeof useCvcTank01LiveScores>; rules: CvcScoringRule[]; slots: any[]; updateSlot: ReturnType<typeof trpc.league.setLineupSlot.useMutation>; canEdit: boolean }) {
...
export function CvcOwnerLineup() {
  ...
  return <div className="space-y-5">
    <div className="rounded-xl border border-white/20 bg-black/25 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-3"><span className="text-sm font-bold uppercase tracking-[0.1em] text-cvc-accent">{displayName} {isMine ? <span className="text-cvc-muted">(my team)</span> : null}</span>
      {overview.data?.franchises.length ? <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.08em] text-cvc-muted">View lineup<select value={viewedFranchiseId} onChange={event => setLocation(mine.data?.id === event.target.value ? "/lineup" : `/lineup/${event.target.value}`)} className="rounded-md border border-white/20 bg-cvc-deep px-2 py-1.5 text-xs font-bold text-white">{mine.data ? <option value={mine.data.id}>{mine.data.name} (my team)</option> : null}{overview.data.franchises.filter(team => team.id !== mine.data?.id).map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label> : null}
    </div></div>
    <div><div className="flex items-center gap-3"><TeamLogo name={displayName} abbreviation={viewedFranchise?.abbreviation} logoUrl={viewedFranchise?.logo_url} size="lg" className="rounded-xl border-cvc-accent/50"/><div><p className="font-display text-4xl uppercase tracking-[0.04em] text-white">{isMine ? "My lineup" : `${displayName} lineup`}</p><p className="mt-1 text-sm text-cvc-muted">{displayName} · {board.data?.week?.label ?? "Current week"} · Players lock at NFL kickoff</p></div></div><div className="mt-5 flex flex-wrap gap-3"><span className="cvc-button-compact opacity-70">Tank01 lineup data</span>{canEdit ? <span className="cvc-button-secondary">Changes save instantly</span> : <span className="cvc-button-secondary opacity-70">View only — {isCommissioner ? "not your franchise" : "commissioner can edit any lineup"}</span>}</div></div>
    {groups.length ? groups.map(group => <SectionTable key={group.key} group={group} profiles={profiles} live={live} rules={rules.data ?? []} slots={slots.data ?? []} updateSlot={updateSlot} canEdit={canEdit}/>) : <section className="rounded-2xl bg-white p-8 text-center text-sm text-slate-600">No active CVC roster assignments are available for this franchise.</section>}
    {canEdit && starterCount === 0 && players.length ? <p className="rounded-xl border border-amber-300/30 bg-amber-50 px-4 py-3 text-sm text-amber-950">No starter slots are assigned yet. Choose a position in each player’s slot control to move them from the bench into the submitted CVC lineup.</p> : null}
