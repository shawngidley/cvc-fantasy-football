...
import { useCvcLineupSeasonStats, type CvcLineupSeasonStats } from "@/hooks/useCvcLineupSeasonStats";
import { useCvcInjuryStatuses, type CvcInjuryStatus } from "@/hooks/useCvcInjuryStatuses";
...
function PlayerIdentity({ player, profile, injuryStatus }: { player: CvcLineupPlayer; profile: Tank01Profile | null | undefined; injuryStatus?: CvcInjuryStatus }) {
  ...
  return <div className="flex items-center gap-2.5"><span className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#edf3ef] text-[10px] font-black text-cvc-deep">{source && !failed ? <img src={source} alt={isDst(player.position) ? `${player.nfl_team ?? "NFL"} team logo` : ""} className={isDst(player.position) ? "h-8 w-8 object-contain" : "h-full w-full object-cover object-top"} onError={() => setFailed(true)} /> : teamInitial(player.display_name)}</span><span><span className="flex items-center gap-1"><Link href={`/player/${player.id}`} className="block whitespace-nowrap text-sm font-bold leading-5 text-cvc-deep hover:text-cvc-accent">{displayName}</Link>{!isDst(player.position) && injuryStatus ? <CvcInjuryBadge status={injuryStatus} /> : null}</span><span className="block whitespace-nowrap text-[11px] font-bold text-slate-500">{isDst(player.position) ? "D/ST" : player.position ?? "—"} · {player.nfl_team ?? "FA"}</span></span></div>;
}
...
function resolveSeasonStats(player: CvcLineupPlayer, seasonStatsByPlayerId: Record<string, CvcLineupSeasonStats>, isCurrentSeasonYear: boolean) {
  const fromNewTable = seasonStatsByPlayerId[player.id];
  if (fromNewTable) return fromNewTable;
  return isCurrentSeasonYear ? (player.seasonStats as unknown as CvcLineupSeasonStats | undefined) ?? null : null;
}

function seasonFantasy(stats: CvcLineupSeasonStats | null | undefined) {
...
function SectionTable({ group, profiles, injuryStatuses, live, projections, rules, slots, pendingSlots, onStageSlot, canEdit, isCommissioner, seasonStatsByPlayerId, isCurrentSeasonYear, onCutPlayer, cutPendingPlayerId, byeWeekByTeam }: { group: CvcLineupGroup; profiles: Record<string, Tank01Profile | null>; injuryStatuses: ReturnType<typeof useCvcInjuryStatuses>; live: ReturnType<typeof useCvcTank01LiveScores>; projections: CvcProjectionMap; rules: CvcScoringRule[]; slots: any[]; pendingSlots: Record<string, string>; onStageSlot: (assignmentId: string, slotCode: string) => void; canEdit: boolean; isCommissioner: boolean; seasonStatsByPlayerId: Record<string, CvcLineupSeasonStats>; isCurrentSeasonYear: boolean; onCutPlayer: (playerId: string, displayName: string | null | undefined) => void; cutPendingPlayerId: string | null; byeWeekByTeam: Record<string, number | null> }) {
  ...
  const renderStats = (stats: CvcLineupSeasonStats | null | undefined) => {
...
export function CvcOwnerLineup() {
  ...
  // Same cached team-schedule source Free Agents already uses for its Bye column --
  // a single DB read instead of every browser fetching and parsing 32 teams' full
  // season schedules live from Tank01.
  const teamScheduleCache = trpc.league.teamScheduleCache.useQuery();
  const byeWeekByTeam = useMemo(() => {
    const map: Record<string, number | null> = {};
    for (const row of teamScheduleCache.data ?? []) map[row.nfl_team] = row.bye_week;
    return map;
  ...
  const [cutPendingPlayerId, setCutPendingPlayerId] = useState<string | null>(null);
  const cutPlayer = trpc.league.cutContractPlayer.useMutation({
    onSuccess: async () => { await Promise.all([utils.league.franchiseRoster.invalidate(), utils.league.activity.invalidate(), utils.league.freeAgents.invalidate()]); },
    onError: error => toast.error(error.message),
    onSettled: () => setCutPendingPlayerId(null),
  });
  const handleCutPlayer = (playerId: string, displayName: string | null | undefined) => {
    if (!viewedFranchiseId) return;
    confirmDialog.confirm({ title: `Cut ${displayName ?? "this player"}?`, description: "This is final, carries no contract penalty, and makes them an unrestricted free agent.", confirmLabel: "Cut", destructive: true, onConfirm: () => { setCutPendingPlayerId(playerId); return cutPlayer.mutateAsync({ franchiseId: viewedFranchiseId, playerId }); } });
  };
  ...
  const projectedTotal = groups.flatMap(group => group.starters).reduce((total, assignment) => total + (assignment.player ? getCvcProjectedPoints(projections, assignment.player.display_name, isDst(assignment.player.position) ? "DST" : assignment.player.position, assignment.player.nfl_team, rules.data ?? []) ?? 0 : 0), 0);
  ...
  // Defaults to the current season; an owner can pick an earlier year to see that
  // season's stats instead. Real component state, not derived fresh from the URL,
  // matching the same pattern already used for the week picker above.
  ...
  const seasonStatsPlayers = useMemo(() => players.flatMap(assignment => {
    if (!assignment.player) return [];
    const espnId = profiles[profileKey(assignment.player.display_name)]?.espnId;
    return [{ playerId: assignment.player.id, espnId, position: isDst(assignment.player.position) ? "DST" : assignment.player.position ?? "" }];
  }), [players, profiles]);
  const seasonStatsByPlayerId = useCvcLineupSeasonStats(seasonStatsPlayers, effectiveStatsYear);
  ...
  return <div className="space-y-5">
    ...
    {injuryStatuses.isError ? <p className="rounded-xl border border-amber-300/30 bg-amber-50 px-4 py-3 text-sm text-amber-950">Injury status is unavailable right now — the absence of a badge below doesn't necessarily mean a player is healthy. Check the Injury Report or the NFL's official report before kickoff.</p> : null}
    {groups.length ? groups.map(group => <SectionTable key={group.key} group={group} profiles={profiles} injuryStatuses={injuryStatuses} live={live} projections={projections} rules={rules.data ?? []} slots={slots.data ?? []} pendingSlots={pendingSlots} onStageSlot={stageSlot} canEdit={canEdit} isCommissioner={isCommissioner} seasonStatsByPlayerId={seasonStatsByPlayerId} isCurrentSeasonYear={effectiveStatsYear === currentSeasonYear} onCutPlayer={handleCutPlayer} cutPendingPlayerId={cutPendingPlayerId} byeWeekByTeam={byeWeekByTeam}/>) : <section className="rounded-2xl bg-white p-8 text-center text-sm text-slate-600">No active CVC roster assignments are available for this franchise.</section>}
