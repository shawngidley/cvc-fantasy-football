...
export function CvcOwnerLineup() {
  ...
  return <div className="space-y-5"><div className="rounded-xl border border-white/20 bg-black/25 px-4 py-3 text-sm font-bold uppercase tracking-[0.1em] text-cvc-accent">{mine.data.name} <span className="text-cvc-muted">(my team)</span></div><div><div className="flex items-center gap-3"><TeamLogo name={mine.data.name} abbreviation={mine.data.abbreviation} logoUrl={mine.data.logo_url} size="lg" className="rounded-xl border-cvc-accent/50"/><div><p className="font-display text-4xl uppercase tracking-[0.04em] text-white">My lineup</p><p className="mt-1 text-sm text-cvc-muted">{mine.data.name} · {board.data?.week?.label ?? "Current week"} · Players lock at NFL kickoff</p></div></div><div className="mt-5 flex flex-wrap gap-3"><span className="cvc-button-compact opacity-70">Tank01 lineup data</span><span className="cvc-button-secondary">Changes save instantly</span></div></div><section className="rounded-2xl border border-cvc-accent/30 bg-[#062412] px-5 py-5 text-white shadow-xl"><div className="grid grid-cols-3 gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/60">Total points</p><p className="mt-1 font-display text-4xl text-cvc-accent">{totalPoints.toFixed(1)}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/60">Starters</p><p className="mt-1 font-display text-4xl">{starterCount}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/60">Live status</p><p className="mt-1 font-display text-2xl text-cvc-accent">{live.isPolling ? "LIVE" : "READY"}</p></div></div><p className="mt-5 text-center text-sm text-white/70">Tank01 updates active games every 30 seconds. Player portraits and season context load from the server-only Tank01 provider path.</p></section>{groups.length ? groups.map(group => <SectionTable key={group.key} group={group} profiles={profiles} live={live} rules={rules.data ?? []} slots={slots.data ?? []} updateSlot={updateSlot}/>) : <section className="rounded-2xl bg-white p-8 text-center text-sm text-slate-600">No active CVC roster assignments are available for this franchise.</section>}{starterCount === 0 && players.length ? <p className="rounded-xl border border-amber-300/30 bg-amber-50 px-4 py-3 text-sm text-amber-950">No starter slots are assigned yet. Choose a position in each player’s slot control to move them from the bench into the submitted CVC lineup.</p> : null}{updateSlot.error ? <p className="text-sm font-medium text-red-700">{updateSlot.error.message}</p> : null}</div>;
}
