...
function SectionTable({ group, profiles, injuryStatuses, live, projections, rules, slots, pendingSlots, onStageSlot, canEdit, isCommissioner, seasonStatsByPlayerId, isCurrentSeasonYear }: { group: CvcLineupGroup; profiles: Record<string, Tank01Profile | null>; injuryStatuses: ReturnType<typeof useCvcInjuryStatuses>; live: ReturnType<typeof useCvcTank01LiveScores>; projections: CvcProjectionMap; rules: CvcScoringRule[]; slots: any[]; pendingSlots: Record<string, string>; onStageSlot: (assignmentId: string, slotCode: string) => void; canEdit: boolean; isCommissioner: boolean; seasonStatsByPlayerId: Record<string, CvcLineupSeasonStats>; isCurrentSeasonYear: boolean }) {
...
export function CvcOwnerLineup() {
  ...
  return <div className="space-y-5">
    ...
    {groups.length ? groups.map(group => <SectionTable key={group.key} group={group} profiles={profiles} injuryStatuses={injuryStatuses} live={live} projections={projections} rules={rules.data ?? []} slots={slots.data ?? []} pendingSlots={pendingSlots} onStageSlot={stageSlot} canEdit={canEdit} isCommissioner={isCommissioner} seasonStatsByPlayerId={seasonStatsByPlayerId} isCurrentSeasonYear={effectiveStatsYear === currentSeasonYear}/>) : <section className="rounded-2xl bg-white p-8 text-center text-sm text-slate-600">No active CVC roster assignments are available for this franchise.</section>}
