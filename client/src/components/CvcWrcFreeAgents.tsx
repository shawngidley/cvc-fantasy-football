...
export function CvcWrcFreeAgents() {
  ...
  return <div className="mx-auto max-w-[1440px]">
    <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
      <div><div className="cvc-eyebrow"><Users size={14} /> Player pool</div><h1 className="mt-2 font-display text-5xl uppercase leading-none tracking-[0.04em] text-white sm:text-6xl">Free Agents & FAAB</h1><p className="mt-3 max-w-3xl text-sm text-cvc-muted">Search the synchronized CVC player pool, review player availability, and submit a protected FAAB claim when the waiver window is open.</p></div>
            ...
            <tbody>
              {isLoading ? <tr><td colSpan={colSpan} className="px-5 py-8 text-center text-sm text-slate-500">Loading CVC player pool…</td></tr>
                ...
                : players.length ? players.map((player: any) => <tr key={player.id} className="border-t border-slate-200 hover:bg-slate-50">
                    ...
                    <td className="px-5 py-2.5 text-right">{player.rosteredByFranchiseName ? <span className="text-xs text-slate-400">—</span> : owner?.franchise && waiver.data?.period ? <button onClick={() => setSelectedPlayerId(player.id)} className="cvc-mini-button"><DollarSign size={13} /> Bid</button> : <span className="text-xs text-slate-400">{owner ? "Window closed" : "Sign in"}</span>}</td>
                  </tr>)
