...
function SeasonStatsSyncModule() {
  ...
  return <div className="grid gap-4">
    ...
    <div className="rounded-lg border border-dashed border-amber-400/40 bg-amber-50 p-4">
      ...
      <pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-all rounded bg-white p-2 text-[10px]">{waiverPeriodCheck.isLoading ? "Loading…" : waiverPeriodCheck.isError ? `ERROR: ${waiverPeriodCheck.error?.message ?? "unknown error"}` : JSON.stringify(waiverPeriodCheck.data, null, 2) || "(empty result)"}</pre>
    </div>
    <div className="rounded-lg border border-dashed border-amber-400/40 bg-amber-50 p-4">
