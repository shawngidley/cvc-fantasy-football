...
export function LeagueLayout({ children }: { children: React.ReactNode }) {
  ...
  const ownerIdentity = teamName ?? owner?.displayName;
  ...
  return <div className="min-h-screen bg-cvc-ink text-foreground"><div className="cvc-stadium-layer" aria-hidden="true" />
    <header className="cvc-nav"><div className="mx-auto flex h-[60px] max-w-[1440px] items-center gap-4 px-4 sm:px-6">
      <Link href="/standings" className="flex shrink-0 items-center gap-2.5"><img src="/brand/header-crest.png" alt="CVC Fantasy Football crest" className="h-9 w-9 object-contain" /><span className="font-display text-base uppercase tracking-[0.08em] text-white sm:text-lg">Fantasy Football</span></Link>
      ...
      <div className="ml-auto flex items-center gap-3">{owner ? <div className="hidden items-center gap-2 sm:flex">{teamName ? <TeamLogo name={teamName} abbreviation={owner.franchise?.abbreviation} logoUrl={owner.franchise?.logo_url} size="xs" className="border-cvc-accent/60"/> : <UserRound size={17} className="text-cvc-accent"/>}<span className="font-display text-sm uppercase tracking-[0.06em] text-cvc-accent">{ownerIdentity}</span>{isCommissioner ? <Link href="/settings" title="Commissioner panel" className={cn("text-white/60 hover:text-cvc-accent", location === "/settings" && "text-cvc-accent")}><ShieldCheck size={18} /></Link> : null}<Link href="/owner-settings" title="Owner settings" className="text-white/60 hover:text-cvc-accent"><Settings size={18} /></Link></div> : <Link href="/login" className="cvc-owner-link"><UserRound size={15} /><span className="hidden sm:inline">Owner sign in</span></Link>}<button className="cvc-menu-button" onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu size={22} /></button></div>
    </div></header>
    {mobileOpen ? <><div className="fixed inset-0 z-[998] bg-black/60" onClick={() => setMobileOpen(false)} /><aside className="fixed right-0 top-0 z-[999] h-full w-[min(86vw,340px)] overflow-y-auto border-l border-cvc-accent/30 bg-cvc-deep px-6 py-6 shadow-2xl"><div className="mb-7 flex items-center justify-between"><p className="font-display text-xl uppercase tracking-[0.08em] text-cvc-accent">CVC menu</p><button onClick={() => setMobileOpen(false)} aria-label="Close menu" className="text-white"><X size={28} /></button></div>{owner ? <Link href="/owner-settings" onClick={() => setMobileOpen(false)} className="mb-4 flex items-center gap-3 rounded-lg border border-cvc-accent/40 bg-cvc-accent/10 p-3 text-cvc-accent"><UserRound size={18}/><span className="font-display text-sm uppercase tracking-[0.08em]">{ownerIdentity} · Owner settings</span></Link> : null}{isCommissioner ? <Link href="/settings" onClick={() => setMobileOpen(false)} className="mb-4 flex items-center gap-3 rounded-lg border border-cvc-accent/40 bg-cvc-accent/10 p-3 text-cvc-accent"><ShieldCheck size={18}/><span className="font-display text-sm uppercase tracking-[0.08em]">Commissioner panel</span></Link> : null}{primaryLinks.map(link => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className={cn("flex items-center gap-2 border-b border-white/10 py-3 font-display text-lg uppercase tracking-[0.08em]", location === link.href ? "text-cvc-accent" : "text-white/85")}>{link.live ? <i className="h-2 w-2 rounded-full bg-red-500" /> : null}{link.label}</Link>)}<DrawerGroup label="Draft" links={draftLinks} location={location} onNavigate={() => setMobileOpen(false)} /><DrawerGroup label="League" links={leagueLinks} location={location} onNavigate={() => setMobileOpen(false)} /></aside></> : null}
