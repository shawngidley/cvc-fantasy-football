...
import { CvcRookieLottery } from "@/components/CvcRookieLottery";
...
// Mirrors the Auction Room's "Correct a pick" panel exactly (same release/release/
// reset/log pattern via correctDraftSelection) -- the rookie draft previously had no
// way to undo a recorded selection at all.
function CorrectRookiePick({ selectedPicks }: { selectedPicks: any[] }) {
  const utils = trpc.useUtils();
  const [correctionId, setCorrectionId] = useState("");
  const correct = trpc.league.correctDraftSelection.useMutation({
    onSuccess: async () => {
      toast.success("Rookie draft selection corrected — pick reopened.");
      setCorrectionId("");
      await Promise.all([utils.league.draftBoard.invalidate(), utils.league.freeAgents.invalidate(), utils.league.eligibleRookies.invalidate(), utils.auction.eligiblePlayers.invalidate()]);
    },
    onError: error => toast.error(error.message),
  ...
  return <section className="mt-5 overflow-hidden rounded-xl border border-white/10 bg-white text-cvc-deep shadow-[0_8px_24px_rgba(0,0,0,0.16)]">
    ...
    <header className="bg-[#123040] px-5 py-3 font-display text-xl uppercase tracking-[0.08em] text-white">Correct a pick</header>
    <div className="p-5">
      <p className="mb-4 text-xs text-slate-500">Undoes a recorded rookie draft selection: releases the player back to the pool, restores the pick to open, and reopens it for a new selection.</p>
      <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
        <select value={correctionId} onChange={event => setCorrectionId(event.target.value)} className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
          <option value="">Choose recent selection</option>
          {selectedPicks.map(pick => <option key={pick.id} value={pick.id}>R{pick.round_number}.{String(pick.pick_number).padStart(2, "0")} — {pick.playerName} · {pick.winningFranchise} · ${pick.salary}</option>)}
        </select>
        <button disabled={!correctionId || correct.isPending} onClick={() => confirmDialog.confirm({ title: "Correct this rookie draft selection?", description: "The player will be released and the pick reopened.", confirmLabel: "Undo selection", destructive: true, onConfirm: () => correct.mutateAsync({ draftPickId: correctionId }) })} className="rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-xs font-black uppercase tracking-[0.1em] text-red-700 disabled:cursor-not-allowed disabled:opacity-50">{correct.isPending ? "Correcting…" : "Undo selection"}</button>
      </div>
      {correct.error ? <p className="mt-2 text-sm text-red-700">{correct.error.message}</p> : null}
    </div>
    {confirmDialog.dialog}
  </section>;
}

function FragmentRow({ pick, showRoundDivider }: { pick: any; showRoundDivider: boolean }) {
  return <>
    {showRoundDivider ? <tr><td colSpan={6} className="border-y-2 border-[#e2b23d]/40 bg-[#faf3e0] px-5 py-2 font-display text-xs uppercase tracking-[0.12em] text-[#a77815]">Round {pick.round_number}</td></tr> : null}
    <tr className="border-b border-slate-100 last:border-0"><td className="px-5 py-4"><span className="rounded bg-[#edf5e7] px-2 py-1 font-display text-sm text-cvc-deep">R{pick.round_number}.{String(pick.pick_number).padStart(2, "0")}</span></td><td className="px-5 py-4 font-semibold">{pick.currentFranchise}{pick.originalFranchise !== "Unknown" && pick.originalFranchise !== pick.currentFranchise ? <span className="ml-1.5 font-normal text-slate-400">({pick.originalFranchise})</span> : null}</td><td className="px-5 py-4">{pick.winningFranchise ? <span className="font-semibold text-emerald-700">{pick.winningFranchise}</span> : <span className="text-slate-400">Awaiting selection</span>}</td><td className="px-5 py-4 font-semibold">{pick.playerName ?? <span className="font-normal text-slate-400">—</span>}</td><td className="px-5 py-4 text-slate-500">{pick.playerNflTeam ?? "—"}</td><td className="px-5 py-4 text-right text-slate-500">{pick.salary !== null ? `$${pick.salary}` : "—"}</td></tr>
  </>;
}
