...
/** Merges the live feed with the archive, deduped by a stable content key -- the live
 * version of an item wins if it appears in both (it's the freshest copy), sorted by
...
/** Snapshots the current live feed into the archive, and prunes anything past its own
 * 30-day expiry. Called once daily by the scheduled job, and additionally (fire-and-
 * forget) on every live request, so the archive stays current even if the daily cron
 * ever misses a run. */
...
export async function getArchivedFantasyProsNews(): Promise<FantasyProsNewsItem[]> {
  ...
  const rows = unwrap(await supabase
    .from("cvc_fantasypros_news_archive")
  ...
  return rows.map(row => archiveRowToNews(row as ArchiveRow));
}
